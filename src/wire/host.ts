...
/**
 * The bridge, and nothing about checklists.
 *
 ...
 * nothing about items or ticks, and the code that knows about those knows
 ...
 * here would be a checklist drawn against another canvas's references.
 *
  ...
  const onMessage = (ev: MessageEvent) => {
    ...
    if (message.type === MESSAGE.CONTEXT) {
      /* Rebuilt field by field rather than passed along, because a context
         ...
         page that shows the diff of what somebody picked and one that shows the
         same empty sentence however hard the canvas is clicked. */
      events.onContext?.({
        epic: message.epic,
