 ...
 * as forty thousand DOM nodes into a pane three hundred pixels wide, the browser
 ...
 * worse than one that locked the pane, because the reader believes it: they
 ...
 *
 * The first file is opened whatever its size, because a pane that opens nothing
 * is a pane that looks like it failed. `FILE_LINES` is what keeps that safe.
 *
...
/** And one file's share. Roughly a long screenful in a narrow pane, several times over. */
...
export interface Plan {
  ...
  closed: number
  /** How many lines of content those closed files hold. Said out loud on the pane. */
  heldBack: number
}
 ...
 * height inside a pane that is being resized by a host. A reader pressing a file
...
export function plan(files: FileDiff[]): Plan {
  ...
  for (const [at, file] of files.entries()) {
    /* The first file always opens. A pane whose every file is shut looks like a
       pane that failed to load one, and `FILE_LINES` already stops the one open
